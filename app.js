...
function montar(){
 ...
 if(quantidade==='poucas'&&tipo!=='odor') passos.splice(2,0,'Use uma dose proporcional à carga pequena; não dose como se o tambor estivesse cheio.');
 ...
 if(sujeira==='pesada'&&!['la','odor'].includes(tipo)){
   ...
   aviso=addAviso(aviso,'Não seque a peça com calor até confirmar que a mancha foi removida.');
 }
 ...
 if(prioridade==='rapido'){
   if(rapido59Elegivel){
     ...
     passos=['Confirme que são poucas peças leves e apenas levemente sujas.','Confira se todas as peças permitem secagem em máquina.','Use uma dose pequena de lava-roupas, proporcional à carga.',"Selecione 59' Lavar + Secar.",'Retire as peças quando o ciclo terminar.'];
     ...
     passos=['Confirme que as peças estão apenas levemente sujas.','Use uma dose pequena de lava-roupas, proporcional à carga.',"Selecione Rápido 15'.",'Não acrescente produtos em excesso em um ciclo tão curto.','Retire as peças ao terminar; este ciclo não seca.'];
 ...
 if(!rapido15Elegivel&&!rapido59Elegivel){
   if(tipo==='toalhas') passos.push(secar==='sim'?'Se for secar na máquina, selecione Secar Algodão e retire as toalhas assim que terminar.':'Retire as toalhas ao fim da lavagem e coloque para secar.');
   if(tipo==='cama') passos.push(secar==='sim'?'Antes de secar, confirme que a carga está dentro do limite de 6 kg; divida se necessário.':'Retire a roupa de cama ao fim da lavagem e seque conforme a etiqueta.');
   if(tipo==='esporte') passos.push(secar==='sim'?'Se a etiqueta permitir, use Secar Sintéticos.':'Retire as peças ao final e prefira secagem ao ar.');
 }

 if(secar==='sim'&&['esporte','la','jeans','pretas'].includes(tipo)&&!rapido59Elegivel) aviso=addAviso(aviso,'A etiqueta da peça decide se a secadora pode ser usada.');
